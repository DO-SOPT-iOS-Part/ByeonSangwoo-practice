...
class ViewController: UIViewController {
    ...
    private func checkUsernameDuplication() {
        Task {
            do {
                let checkNameResult = try await CheckNameService.shared.getCheckNameResultData(usreName: self.usreName)
                
                if checkNameResult?.isExist == true {
                    let alert = UIAlertController(title: "이미 존재하는 username", message: "흑흑", preferredStyle: UIAlertController.Style.alert)
                    let okAction =  UIAlertAction(title: "확인", style: UIAlertAction.Style.default)
                    alert.addAction(okAction)
                    self.present(alert, animated: true)
                } else {
